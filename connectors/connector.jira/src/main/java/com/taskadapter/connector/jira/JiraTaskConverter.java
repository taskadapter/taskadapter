...
public class JiraTaskConverter implements TaskConverter<RemoteIssue> {
	...
	public RemoteIssue convertToJiraIssue(RemoteVersion[] versions,
			RemoteComponent[] components, GTask task) {
		...
		RemoteVersion affectedVersion = getVersion(versions,
				config.getAffectedVersion());
		RemoteVersion fixForVersion = getVersion(versions,
				config.getFixForVersion());
		RemoteCustomFieldValue[] customValues = getCustomFieldsForIssue(config
				.getCustomFields());
		RemoteComponent component = getComponent(components,
				config.getComponent());
	...
	private static RemoteVersion getVersion(RemoteVersion[] versions,
			String versionName) {
	...
	private static RemoteComponent getComponent(RemoteComponent[] objects,
			String name) {
	...
	private RemoteCustomFieldValue[] getCustomFieldsForIssue(
			Map<String, String> configCustomFields) {
		RemoteCustomFieldValue[] values = new RemoteCustomFieldValue[configCustomFields
				.size()];
		int i = 0;
		for (String key : configCustomFields.keySet()) {
			// RemoteField field = getField(possibleCustomFields, key);
			// parentKey : Used for multi-dimensional custom fields such as
			// Cascading select lists. Null in other cases
			String parentKey = null;
			values[i++] = new RemoteCustomFieldValue(key, parentKey,
					new String[] { configCustomFields.get(key) });
		}
		return values;
	}	
	...
	public void setPriorities(RemotePriority[] jiraPriorities) {
		for (int i = 0; i < jiraPriorities.length; i++) {
			priorities.put(jiraPriorities[i].getName(), jiraPriorities[i].getId());
			prioritiesOtherWay.put(jiraPriorities[i].getId(), jiraPriorities[i].getName());
		}
	}
	...
	public String getIssueTypeIdByName(String issueTypeName) {
		...
		if (issueTypeName != null && issueTypeList != null) {
			for (int i = 0; i < issueTypeList.length; i++) {
				if (issueTypeList[i].getName().equals(issueTypeName)) {
					resTypeId = issueTypeList[i].getId();
					break;
				}
			}
		}
	...
	public String getIssueTypeNameById(String issueTypeId) {
		...
		if (issueTypeList != null) {
			for (int i = 0; i < issueTypeList.length; i++) {
				if (issueTypeList[i].getId().equals(issueTypeId)) {
					resTypeName = issueTypeList[i].getName();
					break;
				}
			}
		}
