...
class Storage(storageFolder: File, fileNamePrefix: String, fileNameExtention: String) {
  ...
  def storeNewItemWithAutogeneratedName[T](result: T): Unit = {
    val file = FileNameGenerator.createSafeAvailableFile(storageFolder, s"${fileNamePrefix}-%d.$fileNameExtention", 10000000)
    store(result, file.getName)
  }

  def store[T](result: T, elementId: String): Unit = {
    storageFolder.mkdirs()
    val relativeFileName = s"$fileNamePrefix-$elementId.$fileNameExtention"
    val file = new File(storageFolder, relativeFileName)
